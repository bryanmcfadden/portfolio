(function($,wp,wps,window,undefined){'$:nomunge';var $w=$(window),eventName='waypoint.reached',triggerWaypoint=function(way,dir){way.element.trigger(eventName,dir);if(way.options.triggerOnce){way.element[wp]('destroy')}},waypointIndex=function(el,context){if(!context)return-1;var i=context.waypoints.length-1;while(i>=0&&context.waypoints[i].element[0]!==el[0]){i-=1}
return i},contexts=[],Context=function(context){$.extend(this,{element:$(context),oldScroll:0,'waypoints':[],didScroll:!1,didResize:!1,doScroll:$.proxy(function(){var newScroll=this.element.scrollTop(),isDown=newScroll>this.oldScroll,that=this,pointsHit=$.grep(this.waypoints,function(el,i){return isDown?(el.offset>that.oldScroll&&el.offset<=newScroll):(el.offset<=that.oldScroll&&el.offset>newScroll)}),len=pointsHit.length;if(!this.oldScroll||!newScroll){$[wps]('refresh')}
this.oldScroll=newScroll;if(!len)return;if(!isDown)pointsHit.reverse();$.each(pointsHit,function(i,point){if(point.options.continuous||i===len-1){triggerWaypoint(point,[isDown?'down':'up'])}})},this)});$(context).bind('scroll.waypoints',$.proxy(function(){if(!this.didScroll){this.didScroll=!0;window.setTimeout($.proxy(function(){this.doScroll();this.didScroll=!1},this),$[wps].settings.scrollThrottle)}},this)).bind('resize.waypoints',$.proxy(function(){if(!this.didResize){this.didResize=!0;window.setTimeout($.proxy(function(){$[wps]('refresh');this.didResize=!1},this),$[wps].settings.resizeThrottle)}},this));$w.load($.proxy(function(){this.doScroll()},this))},getContextByElement=function(element){var found=null;$.each(contexts,function(i,c){if(c.element[0]===element){found=c;return!1}});return found},methods={init:function(f,options){this.each(function(){var cElement=$.fn[wp].defaults.context,context,$this=$(this);if(options&&options.context){cElement=options.context}
...
else{$.error('Method '+method+' does not exist on jQuery '+wp)}};$.fn[wp].defaults={continuous:!0,offset:0,triggerOnce:!1,context:window};$[wps]=function(method){if(jQMethods[method]){return jQMethods[method].apply(this)}
else{return jQMethods.aggregate()}};$[wps].settings={resizeThrottle:200,scrollThrottle:100};$w.load(function(){$[wps]('refresh')})})(jQuery,'waypoint','waypoints',window)
