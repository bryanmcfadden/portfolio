...
**/
	...
	//get menu selection for menu setup
	var menuSelection = function (){
		var menu_items = new Array();
		if((jQuery.cookie) && (jQuery.cookie("menu_selection")!='') && (jQuery.cookie("menu_selection")!=undefined)){
			menu_items = jQuery.cookie("menu_selection").split('|');
		}else{
			menu_items[0] = 'My Work';
		}
		return menu_items;
	}
	
	//set sidebar menu
	var setSidebarMenu = function (){
		// ------------------------------------------------------------------------------------------------------------------------------------------------
		var mnu = menuSelection();
		//get each list item inthe menu and iterate through to set values
		jQuery('.page-sidebar-menu').children('li').each(function(){
			if(jQuery(this).children('a').attr('title')!=undefined){
				//is this the main menu item selected?
				if(jQuery(this).children('a').attr('title')==mnu[0]){
					//set selected
					jQuery(this).addClass('active');
					jQuery(this).find('span.arrow').addClass('open');
					jQuery(this).find('span').last().addClass('selected');
				}else{
					//clear
					jQuery(this).removeClass('active');
					jQuery(this).find('arrow.open').removeClass('open');
					jQuery(this).find('span').last().removeClass('selected');
				}
			}
		});
		// ------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    var handleSidebarMenu = function () {
        jQuery('.page-sidebar').on('click', 'li > a', function (e) {
			var sel;
			//Title - alert(jQuery(this).attr('title'));
			//Parent Title - (jQuery(par[0]).prev().attr('title'));
			//get menu item level - var par = jQuery(this).parents('ul');
			var par = jQuery(this).parents('ul');
			var title = jQuery(this).attr('title');
			var path = '';
			var count = (par.length-1);
			//for debugging to collect path in menu tree
			//alert('par length: ' + par.length + '\npar[0] :' + 
			//		jQuery(par[0]).prev().attr('title') + '\npar[1] :' + 
			//		jQuery(par[1]).prev().attr('title') + '\npar[2] :' + 
			//		jQuery(par[2]).prev().attr('title'));
			for(var i=count; i>=0; i--){
				if(jQuery(par[i]).prev().attr('title')!=undefined){
					path+= jQuery(par[i]).prev().attr('title')+'|';
				}
			}
			//add selected item
			path+=title;
			//TO DO:
	//alert(path);
			//1. Store path in cookie
			if(jQuery.cookie){
				jQuery.cookie('menu_selection', path);
			}
			
			//2. Clear all items & sub-menu items 
			//3. Set seleced menu item class to 'active'
			//3.  and span needs to have class='selected'
			//4. set sub-menu-items
            if (jQuery(this).next().hasClass('sub-menu') == false) {
			
				// Added by BM ------------ 3/10/2014 - find selected and reset
				/*
				jQuery('.page-sidebar-menu').children('li').each(function(){
					jQuery(this).removeClass('active');
				});
            ...
            var parent = jQuery(this).parent().parent(); //get parent list item
            var the = jQuery(this); // this clicked <a>
			
            parent.children('li.open').children('a').children('.arrow').removeClass('open'); // close 
            ...
            if (sub.is(":visible")) {
				...
				// Added by BM ------------ 3/7/2014
					//find selected item and remove active
					/*
	...
	//NEED TO REWRITE FOR PAGE CONTAINER DIV IN PORTAL8-----------------------------------------------
	//------------------------------------------------------------------------------------------------
    // Handles sidebar toggler to close/hide the sidebar.
    var handleSidebarToggler = function () {
        var viewport = _getViewPort();
        if (jQuery.cookie && jQuery.cookie('sidebar_closed') === '1' && viewport.width >= 992) {
	...
	*/
    ...
    var handleTheme = function () {
        ...
        if ( jQuery('.sidebar-pos-option').attr("disabled") === false) {
            jQuery('.sidebar-pos-option', panel).val(App.isRTL() ? 'right' : 'left');
        }
        ...
        var setLayout = function () {
            ...
            if (App.isRTL()) {
                if (sidebarPosOption === 'left') {
                    ...
                    jQuery('#frontend-link').tooltip('destroy').tooltip({placement: 'right'});
                } else {
                    jQuery("body").removeClass("page-sidebar-reversed");
                    jQuery('#frontend-link').tooltip('destroy').tooltip({placement: 'left'});
                }
            } else {
        ...
        var setColor = function (color) {
            ...
            jQuery('#style_color').attr("href", "assets/css/themes/" + color_ + ".css");
            if(jQuery.cookie) {
        ...
        jQuery('.layout-option, .header-option, .sidebar-option, .footer-option, .sidebar-pos-option', panel).change(setLayout);

        if (jQuery.cookie && jQuery.cookie('style_color')) {
    ...
    return {
        ...
        init: function () {
			setSidebarMenu(); //set selected values for menu
